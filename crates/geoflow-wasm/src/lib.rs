...
/// Return the canonical semantic graph projection for the AGS file.
#[wasm_bindgen]
pub fn get_semantic_graph(bytes: &[u8]) -> String {
    let file = ags::parse_bytes(bytes).file;
    serde_json::to_string(&geoflow_core::semantic::build_semantic_graph(&file)).unwrap_or_default()
}

/// Return all investigation point IDs containing a given explicit GEOL_LEG code.
#[wasm_bindgen]
