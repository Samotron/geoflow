...
type ColorField = 'LOCA_ID' | 'GEOL_GEOL' | 'GEOL_DESC' | 'GEOL_LEG';
...
function geolColor(key: string): string {
...
function colorScaleFor(data: { colorKey: string }[]) {
...
function getGeolLayers(file: AgsFile): GeolLayer[] {
...
function getGeolFields(file: AgsFile): ColorField[] {
...
function ckAtDepth(locaId: string, depth: number | null, layers: GeolLayer[], field: ColorField): string {
...
function extractLocaMap(file: AgsFile): Map<string, number | null> {
...
function extractAllLocaIds(file: AgsFile): string[] {
  return (file.groups['LOCA']?.rows ?? []).map(r => str(r, 'LOCA_ID')).filter(Boolean);
}

function extractSptDepth(file: AgsFile, layers: GeolLayer[], cf: ColorField, bh: Set<string>): DepthDatum[] {
...
function extractSptElev(file: AgsFile, locaMap: Map<string, number | null>, layers: GeolLayer[], cf: ColorField, bh: Set<string>): ElevDatum[] {
  return (file.groups['ISPT']?.rows ?? []).flatMap(r => {
    const locaId = str(r, 'LOCA_ID');
    if (!locaId || (bh.size > 0 && !bh.has(locaId))) return [];
    const depth = num(r, 'ISPT_DPTH'), n = num(r, 'ISPT_NVAL');
...
function extractAtterberg(file: AgsFile, layers: GeolLayer[], cf: ColorField, bh: Set<string>): XYDatum[] {
...
function extractPsd(file: AgsFile, layers: GeolLayer[], cf: ColorField, bh: Set<string>): PsdPoint[] {
  const pts: PsdPoint[] = [];
  for (const [grp, szF, pcF] of [['GRAG','GRAG_SIZO','GRAG_PCPG'],['SIEV','SIEV_SIZE','SIEV_PCPG']] as const) {
    for (const r of file.groups[grp]?.rows ?? []) {
      ...
      // Heuristic: SIEV_SIZE values 1–200 are typically μm (AGS3 legacy), convert to mm
...
function extractDepth(file: AgsFile, grp: string, vF: string, dF: string, layers: GeolLayer[], cf: ColorField, bh: Set<string>): DepthDatum[] {
...
function extractUu(file: AgsFile, layers: GeolLayer[], cf: ColorField, bh: Set<string>): DepthDatum[] {
  ...
  // TNPC - pocket penetrometer / pilcon
...
function extractDensity(file: AgsFile, layers: GeolLayer[], cf: ColorField, bh: Set<string>): { bulk: DepthDatum[]; dry: DepthDatum[] } {
...
function extractShear(file: AgsFile, layers: GeolLayer[], cf: ColorField, bh: Set<string>): XYDatum[] {
...
function extractCompaction(file: AgsFile, layers: GeolLayer[], cf: ColorField, bh: Set<string>): CmpnPoint[] {
...
function extractCong(file: AgsFile, layers: GeolLayer[], cf: ColorField, bh: Set<string>): XYDatum[] {
...
function OPlot({ spec }: { spec: PlotSpec }) {
  ...
  useEffect(() => {
    ...
    // Make the output SVG responsive
    if (chart instanceof SVGElement) {
      ...
      // Figure wrapper (present when legend:true)
...
function sptDepthSpec(data: DepthDatum[]): PlotSpec {
...
function sptElevSpec(data: ElevDatum[]): PlotSpec {
  const cs = colorScaleFor(data);
  const maxN = Math.max(60, ...data.map(d => d.value));
  return {
    ...ELEV_GRID_STYLE,
...
function plasticitySpec(data: XYDatum[]): PlotSpec {
  const cs = colorScaleFor(data);
...
function psdSpec(data: PsdPoint[]): PlotSpec {
  const cs = colorScaleFor(data);
  // British Standard grain size boundaries (mm)
  const boundaries = [
  ...
  return {
    ...
    marks: [
      ...
      // Grading curves (lines per sample, sorted by size)
      Plot.line(
...
function depthScatterSpec(data: DepthDatum[], xLabel: string, xDomain?: [number, number]): PlotSpec {
  const cs = colorScaleFor(data);
  return {
    ...DEPTH_GRID_STYLE,
...
interface DensityDatum extends DepthDatum { type: 'Bulk' | 'Dry'; }

function densitySpec(bulk: DepthDatum[], dry: DepthDatum[]): PlotSpec {
...
function shearBoxSpec(data: XYDatum[]): PlotSpec {
  const cs = colorScaleFor(data);
...
function compactionSpec(data: CmpnPoint[]): PlotSpec {
  const cs = colorScaleFor(data);
  return {
    height: 360,
    marginLeft: 55,
    ...
    marks: [
      Plot.line(
        ...
        { x: 'mc', y: 'dden', z: 'curveId', stroke: d => (d as CmpnPoint).colorKey, strokeWidth: 1.5, curve: 'catmull-rom', tip: false }
      ),
...
function congSpec(data: XYDatum[]): PlotSpec {
  const cs = colorScaleFor(data);
  return {
    height: 360,
    marginLeft: 55,
...
type PlotId = (typeof ALL_PLOTS)[number]['id'];
...
export function PlotsTab({ fileBytes }: Props) {
  ...
  // ── Memoised specs (stable references → OPlot won't re-render unnecessarily) ──
  ...
  return (
    <div style={{ display: 'flex', gap: 0, minHeight: '70vh' }}>
      ...
      <aside style={{ width: 230, flexShrink: 0, borderRight: '1px solid var(--border)', padding: '16px 14px', overflowY: 'auto', background: '#fafbfc', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        {/* Borehole filter (only when ≤ 30 boreholes) */}
      ...
      <main style={{ flex: 1, padding: '16px 20px', overflowY: 'auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(480px, 1fr))', gap: 18 }}>
          ...
          {activePlots.has('spt_depth') && (
            <Card title="SPT N-value vs Depth" n={dataN.spt_depth}>
              {specSptDepth && <OPlot key="spt_depth" spec={specSptDepth} />}
            </Card>
          )}

          {activePlots.has('spt_elev') && (
            <Card title="SPT N-value vs Level (mOD)" n={dataN.spt_elev}>
              {dataN.spt_elev === 0
                ? null
                : specSptElev
          ...
          {activePlots.has('plasticity') && (
            <Card title="Plasticity Chart — A-line" n={dataN.plasticity}>
