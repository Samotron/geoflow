...
function extractGeolSummary(file: AgsFile): GeolUnit[] {
  ...
  for (const r of file.groups['GEOL']?.rows ?? []) {
    ...
    if (existing) {
      ...
      map.set(code, {
        code, desc, count: 1,
        minTop: top ?? Infinity, maxBase: base ?? -Infinity,
...
function sptStats(file: AgsFile) {
  return computeStats((file.groups['ISPT']?.rows ?? []).flatMap(r => {
    const v = num(r, 'ISPT_NVAL'); return v != null ? [v] : [];
  }));
}

interface AtterbergStats { ll: Stats | null; pl: Stats | null; pi: Stats | null; }
...
function mcStats(file: AgsFile) {
  return computeStats((file.groups['LNMC']?.rows ?? []).flatMap(r => {
...
function cuStats(file: AgsFile) {
  ...
  return computeStats(extractUu(file, [], 'LOCA_ID', EMPTY).map(d => d.value));
}
...
function psdStats(file: AgsFile): PsdStats | null {
  ...
  function addPassing(sizeVal: number, pc: number, isLegacy: boolean) {
    ...
    const key = sz <= 0.002 ? 'clay' : sz <= 0.063 ? 'fine' : sz <= 2 ? 'medium' : 'coarse';
...
function StatsRow({ label, s, unit, dp = 1 }: { label: string; s: Stats | null; unit: string; dp?: number }) {
  ...
  return (
    <tr>
      ...
      {[s.n, fmt(s.min, dp), fmt(s.max, dp), fmt(s.mean, dp), fmt(s.std, dp)].map((v, i) => (
        <td key={i} style={{ ...S.td, fontVariantNumeric: 'tabular-nums', textAlign: 'center' }}>{v}</td>
      ))}
    </tr>
  );
}
...
export function ReportTab({ fileBytes, fileName }: Props) {
  ...
  // Plot data — same extractors as PlotsTab, coloured by geology code
  ...
  const sptDepth   = useMemo(() => agsFile ? extractSptDepth(agsFile, layers, colorField, EMPTY_BH) : [], [agsFile, layers, colorField, EMPTY_BH]);
  const sptElev    = useMemo(() => agsFile ? extractSptElev(agsFile, locaMap, layers, colorField, EMPTY_BH) : [], [agsFile, locaMap, layers, colorField, EMPTY_BH]);
  const atterberg  = useMemo(() => agsFile ? extractAtterberg(agsFile, layers, colorField, EMPTY_BH) : [], [agsFile, layers, colorField, EMPTY_BH]);
  const psd        = useMemo(() => agsFile ? extractPsd(agsFile, layers, colorField, EMPTY_BH) : [], [agsFile, layers, colorField, EMPTY_BH]);
  const llplDepth  = useMemo(() => agsFile ? extractLlplDepth(agsFile, layers, colorField, EMPTY_BH) : [], [agsFile, layers, colorField, EMPTY_BH]);
  const moisture   = useMemo(() => agsFile ? extractDepth(agsFile, 'LNMC', 'LNMC_MC', 'SAMP_TOP', layers, colorField, EMPTY_BH) : [], [agsFile, layers, colorField, EMPTY_BH]);
  const cu         = useMemo(() => agsFile ? extractUu(agsFile, layers, colorField, EMPTY_BH) : [], [agsFile, layers, colorField, EMPTY_BH]);
  const density    = useMemo(() => agsFile ? extractDensity(agsFile, layers, colorField, EMPTY_BH) : { bulk: [], dry: [] }, [agsFile, layers, colorField, EMPTY_BH]);
  const shear      = useMemo(() => agsFile ? extractShear(agsFile, layers, colorField, EMPTY_BH) : [], [agsFile, layers, colorField, EMPTY_BH]);
  const compaction = useMemo(() => agsFile ? extractCompaction(agsFile, layers, colorField, EMPTY_BH) : [], [agsFile, layers, colorField, EMPTY_BH]);
  const cong       = useMemo(() => agsFile ? extractCong(agsFile, layers, colorField, EMPTY_BH) : [], [agsFile, layers, colorField, EMPTY_BH]);

  // Specs — identical functions as PlotsTab
  ...
  const hasAnyPlot  = ALL_PLOTS.some(p => dataN[p.id] > 0);
  ...
  return (
    <div>
      ...
      {/* ── Report body ── */}
      <div ref={reportRef} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '32px 40px' }}>
        ...
        <div style={{ marginBottom: 40, borderBottom: '3px solid var(--navy)', paddingBottom: 24 }}>
          ...
          {proj?.projLoc && <div style={{ fontSize: 15, color: 'var(--muted)', marginBottom: 16 }}>{proj.projLoc}</div>}
        ...
        <div style={S.section} className="report-section">
          ...
          {sampSum.length > 0 && (
            <>
              ...
              <table style={S.table}>
                <thead><tr>{['Type','Description','Count'].map(h => <th key={h} style={S.th}>{h}</th>)}</tr></thead>
                <tbody>
                  {sampSum.map((s, i) => (
                    <tr key={s.type}>
                ...
                <tbody>
                  {loca.map((l, i) => (
                    <tr key={l.id}>
                      ...
                      <td style={i % 2 ? S.tdAlt : S.td}><span style={S.pill('var(--blue)')}>{l.type || '—'}</span></td>
        ...
        {hasAnyStats && (
          <div style={S.section} className="report-section">
            ...
            <p style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 14 }}>
              n = number of results; Std Dev = sample standard deviation.
            </p>

            {spStat && (
              <><h3 style={S.h3}>Standard Penetration Test (SPT)</h3>
              <StatsTable><StatsRow label="SPT N-value" s={spStat} unit="blows/300mm" dp={1} /></StatsTable></>
            )}
            ...
            {mcStat && (
              ...
              <StatsTable><StatsRow label="Natural moisture content (w)" s={mcStat} unit="%" /></StatsTable></>
            )}

            {cuStat && (
              <><h3 style={S.h3}>Undrained Shear Strength (TCON / VANE / TNPC)</h3>
              <StatsTable><StatsRow label="Cu" s={cuStat} unit="kPa" /></StatsTable></>
            )}
            ...
            {psdStat && (
              ...
              <table style={S.table}>
                <thead><tr>{['Fraction','Boundary','Average % passing'].map(h => <th key={h} style={S.th}>{h}</th>)}</tr></thead>
                <tbody>
                  {[
